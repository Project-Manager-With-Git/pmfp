...
def delete(argv: Sequence[str]) -> None:
    """使用DELETE方法访问一个http资源.

    ppm http delete [-flags] url
    """
    parser = argparse.ArgumentParser(
        prog='ppm http delete',
        description='使用delete方法访问http服务器上的资源',
        usage=ppm_http.subcmds.get("delete").__doc__
    )
    ...
    parser.add_argument("url", type=str,
                        help="url地址")
    parser.set_defaults(func=cmd_delete_http)
    args = parser.parse_args(argv)
    args.func(args)


def cmd_delete_http(args: argparse.Namespace) -> None:
    """Delete query."""
    http_query(
        url=args.url,
        method="GET",
