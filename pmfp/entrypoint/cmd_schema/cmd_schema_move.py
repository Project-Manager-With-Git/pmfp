...
def move(argv:Sequence[str])->None:
    ...
    parser.add_argument("-p", "--path", type=str, help="相对根目录的路径")
    parser.add_argument("-v", "--version", type=str, help="模式的版本")
    parser.add_argument("-r", "--root", type=str, help="存放的根地址") 
    parser.add_argument("-a", "--addr", type=str, help="网址") 
    parser.add_argument("-n","--name",type=str, help="schema名")
    parser.add_argument("--remove_old", action="store_true", help="是否删除旧的json schema模式文件")
    parser.add_argument('-o',"--old_root", type=str,required=True, help="旧schema的根目录地址")
    parser.add_argument("file",type=str, help="旧json schema的路径,如果路径指向一个文件夹则遍历文件夹执行相同的操作.")
