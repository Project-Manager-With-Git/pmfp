...
def static(argv:Sequence[str])->None:
    ...
    parser.add_argument("-l", "--language", type=str,choices=("py",), help="静态类型检验针对的语言.")
    parser.add_argument("-m", "--model",action="store_true", help="静态类型检验针对是模块还是执行脚本.")
    parser.add_argument("-c", "--coverage",action="store_true", help="静态类型检验是否输出覆盖率报告.")
    parser.add_argument("-o", "--output", type=str,default="typecheck", help="静态类型检验结果输出位置.")
    parser.add_argument("code",type=str, help="指定要测的项目代码")
