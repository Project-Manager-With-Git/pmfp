...
def build_pb_js(files: List[str], includes: List[str], to: str, grpc: bool,
    ...
    if grpc:
        ...
        并将插件`grpc_node_plugin`的路径指定到环境变量`PROTOC_GEN_GRPC_JS_PATH`""")
        task = "grpc"
    ...
    run_command(
        ...
        lambda x: print(f"编译{task}项目 {target_str} 为js语言模块完成!")
    ).catch(
        lambda content :warnings.warn(f"""编译{task}项目 {target_str} 为python模块失败:
