...
def benchmark_test(language: str, code: str, *, cwd: Optional[str] = None, mem: bool = False) -> None:
    """对指定语言的代码做性能测试.
    ...
    """
    if language == "py":
        benchmark_test_py(code=code, mem=mem, cwd=cwd)
    elif language == "go":
        benchmark_test_go(code=code, mem=mem, cwd=cwd)
    else:
