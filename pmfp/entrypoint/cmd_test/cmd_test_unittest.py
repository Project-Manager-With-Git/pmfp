...
def unittest(argv: Sequence[str]) -> None:
    ...
    parser.add_argument("--root", type=str, help="单元测试执行的位置.")
...
def cmd_unittest_test(args: argparse.Namespace) -> None:
    """检测动态语言的类型."""
    unittest_test(language=args.language, testcode=args.testcode,
                  source=args.source, coverage=args.coverage, output=args.output, root=args.root)
