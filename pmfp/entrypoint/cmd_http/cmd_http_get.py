...
def get(argv: Sequence[str]) -> None:
    ...
    ppm http get [-flags] url
    """
    parser = argparse.ArgumentParser(
        prog='ppm http get',
        description='使用get方法访问http服务器上的资源',
        usage=ppm_http.subcmds.get("get").__doc__
    )
    ...
    parser.add_argument("-t", "--payload_type", type=str,
                        choices=("form", "json", "url", "stream"), default="json",
                        help="负载的类型")
    ...
    parser.add_argument("url", type=str,
                        help="url地址")
    parser.set_defaults(func=cmd_get_http)
    args = parser.parse_args(argv)
    args.func(args)


def cmd_get_http(args: argparse.Namespace) -> None:
    """Get Query."""
    http_query(
        url=args.url,
        method="GET",
