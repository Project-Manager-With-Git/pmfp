...
def _build_pb_cpp(files: List[str], includes: List[str], to: str, cwd: Path, **kwargs: str) -> None:
    includes_str = " ".join([f"-I {include}" for include in includes])
    target_str = " ".join(files)
    flag_str = ""
    if kwargs:
        flag_str += " ".join([f"{k}={v}" for k, v in kwargs.items()])
    command = f"protoc  {includes_str} {flag_str} --cpp_out={to} {target_str}"
    try:
...
def build_pb_cpp(files: List[str], includes: List[str], to: str, cwd: Path,
    ...
    """
    _build_pb_cpp(files, includes, to, cwd, **kwargs)
