...
def push_docker(url=None):
    ...
    if url == None:
        ...
        subprocess.check_call(['docker','push',info["project_name"]+":"+info["version"]])
        print("push the img to docker done!")
    else:
        info = project_info()
        print("push the img to "+url)
        subprocess.check_call(['docker','tag',info["project_name"]+":"+info["version"],url+info["project_name"]+":"+info["version"]])
        subprocess.check_call(['docker','push',url+info["project_name"]+":"+info["version"]])
